 ...
 */
class UserSearchEntityListToUserSearchModelListMapper : BaseMapper<List<UserSearchEntity>, List<UserSearchModel>>() {
    override fun map(value: List<UserSearchEntity>): List<UserSearchModel> {
        return value.map {
            UserSearchModel(
                id = it.id,
